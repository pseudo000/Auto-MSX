...
#이 다음단계에서 complete 여부 과정을 추가. 실패시 프로그램 멈춤
        image_location = pyautogui.locateOnScreen('COMPLETE22.png')
        if image_location is None:
            messagebox.showerror('ERROR', '이미지가 발견되지 않았습니다. 프로그램을 종료합니다.')
            return   
        time.sleep(5)
        ...
        # pyautogui.hotkey("Alt", "f4")
        # time.sleep(1)
#프로그램이 정상 완료된 경우, file_name의 txt, pdf를 별도 폴더(완료폴더)로 이동처리
